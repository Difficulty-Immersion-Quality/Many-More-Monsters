...
if Ext.IsClient() then
    ...
    local function recursiveFindForHitChance(parent, nodeName)
        local success, name = pcall(function(...)
            return parent:GetProperty("Name")
        end)

        if success and name == nodeName then
            return parent
        else
            ...
            if success and childrenCount then
                for i = 1, childrenCount do
                    local node = recursiveFindForHitChance(parent:Child(i), nodeName)
                    if node then
    ...
    Ext.RegisterConsoleCommand("Lab_FindNode", function(cmd, ...)
        local root = recursiveFindForHitChance(Ext.UI.GetRoot():Child(1):Child(1), ({ ... })[1])
        Logger:BasicInfo("%s", root and childPath or "Not Found")
    end)

    Channels.LogToPlayerOverhead:SetHandler(function(message, user)
        local print_info                         = { Duration = 6, Text = message}

        local root                               = recursiveFindForHitChance(Ext.UI.GetRoot(), "OverheadInfo")
        root.DataContext.CurrentSubtitleDuration = print_info.Duration
        root.DataContext.CurrentSubtitle         = print_info.Text
    end)
end
