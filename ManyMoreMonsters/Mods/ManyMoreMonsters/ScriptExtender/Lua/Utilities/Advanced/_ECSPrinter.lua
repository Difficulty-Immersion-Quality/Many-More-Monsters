...
---@type Logger
ChangePrinter.logger = Logger:new("ECSPrinter.txt")
ChangePrinter.logger:ClearLogFile()
---@class ECSPrinter_Listener
ChangePrinter.listener = {
	---@type fun(self, entity: EntityHandle, frame: number)
	onTick = function() end,
	onStop = function() end,
	---@type fun(self, message: string, frame: number, updateEntitySummary: boolean?)
...
function ChangePrinter:Start(frameLimit, ...)
	...
	if not self.TickHandler then
		self.FrameLimit = frameLimit or 1000
		...
		self.logger:BasicInfo("Started Tracing")
	end
end
...
function ChangePrinter:Stop()
	...
	self.logger:BasicInfo("Finished Tracing")
...
function ChangePrinter:OnTick()
	...
	for entity, changes in pairs(trace.Entities) do
		if ((not next(self.TargetedEntities) or not entity.Uuid) or (entity.Uuid and TableUtils:IndexOf(self.TargetedEntities, entity.Uuid.EntityUuid))) then
			if self:EntityHasPrintableChanges(entity, changes) then
				local msg, shouldUpdateSummary

				if self.PrintChanges then
					msg = self:GetEntityNameDecorated(entity) .. ": "
					if changes.Create then msg = msg .. " Created" end
					if changes.Destroy then msg = msg .. " Destroyed" end
					self.logger:BasicDebug("[#" .. self.FrameNo .. "] " .. msg)
				end
				...
				for _, component in pairs(changes.Components) do
					if self:IsComponentChangePrintable(entity, component) then
						...
						if self.PrintChanges then
							local componentMsg = "\t" .. component.Name .. ": "
							if component.Create then componentMsg = componentMsg .. " Created" end
							if component.Destroy then componentMsg = componentMsg .. " Destroyed" end
							if component.Replicate then componentMsg = componentMsg .. " Replicated" end
							self.logger:BasicDebug(componentMsg)
							msg = msg .. "\n" .. componentMsg

							if not shouldUpdateSummary then
								shouldUpdateSummary = entity:HasRawComponent("eoc::BoostInfoComponent")
									or entity:HasRawComponent("esv::status::StatusComponent")
									or entity:HasRawComponent("eoc::PassiveComponent")
							end
						end
					end
				end
				self.listener:message(msg, self.FrameNo, shouldUpdateSummary)
			end

			self.listener:onTick(entity, self.FrameNo)
		end
	end
...
function SpellPrinter:OnTick()
				...
				if self:IsComponentChangePrintable(entity, component) then
					...
					if component.Name == "ecl::spell_preview::EffectsComponent" then
						Ext.Events.Tick:Subscribe(function()
							self.logger:BasicDebug(entity:GetAllComponents())
						end)
					end
				end
			end
	...
	Ext.Entity.ClearTrace()
	self.FrameNo = self.FrameNo + 1
	self.logger.frame = self.logger.frame + 1
end
